...
# Сполучники/прийменники, які в назвах свят пишемо з малої літери.
TITLE_STOP = {
    "і", "й", "та", "а", "в", "у", "на", "до", "з", "с", "за", "об",
    "о", "про", "при", "по", "для", "без", "від", "над", "під",
    "через", "після", "перед", "між", "біля", "крім", "як", "що",
    "храм",
}
# Скорочення, які залишаємо з малої літери всередині назви («св. Іван»).
ABBR_RE = re.compile(r"^(св|свв|апп|ап|єп|свт|мч|прп|блж|прор)\.$")


def cap_first_letter(word: str) -> str:
    """«(пасха» -> «(Пасха», «духа.» -> «Духа.» — велика на першій літері."""
    return re.sub(r"(\W*)(\w)", lambda m: m.group(1) + m.group(2).upper(),
                  word, count=1)


def title_case(name: str) -> str:
    """«різдво господнє» -> «Різдво Господнє».

    Кожне значуще слово з великої; сполучники/прийменники та скорочення
    («св.», «апп.», «свт.») лишаються з малої; перше слово — завжди з великої.
    """
    out = []
    words = name.split()
    for i, w in enumerate(words):
        ...
        if wl in TITLE_STOP:
            ...
            out.append(w.upper())
        elif i == 0 and ABBR_RE.match(wl):
            out.append(cap_first_letter(w))
        elif ABBR_RE.match(wl):
            out.append(w)
        else:
            out.append(cap_first_letter(w))
    return " ".join(out)
...
def clean_name(name: str) -> str:
    """Нормалізує назву празника (пробіли, &nbsp;, переноси PDF, регістр)."""
    name = re.sub(r"&nbsp;|\u00a0", " ", name)
    # Склейка слів, розірваних переносом рядка у PDF: «гос - поднє» -> «господнє»
    name = re.sub(r"\s*-\s*", "", name)
    name = re.sub(r"\s+", " ", name).strip()
    low = name.lower()
    low = re.sub(r"гніх", "Господнє", low)  # артефакт HTML календаря УГКЦ
    low = re.sub(
    ...
    low = re.sub(r"господа нашого ісуса христа", "Господнє", low)
    low = re.sub(r"чесного і животворного хреста", "Чесного Хреста", low)
    low = re.sub(r"святих першоверховних апостолів петра і павла",
                 "Свв. апп. Петра і Павла", low)
